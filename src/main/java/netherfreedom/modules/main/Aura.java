...
import netherfreedom.utils.TimerUtils;
import netherfreedom.utils.NFPlayerUtils;
...
public class Aura extends Module {
    ...
    public Aura() {
        super(NetherFreedom.MAIN, "aura", "Attacks entites around you.");
    }
    ...
    private boolean entityCheck(Entity entity) {
        ...
        if (NFPlayerUtils.distanceFromEye(entity) > range.get()) return false;
        if (!entities.get().getBoolean(entity.getType())) return false;
        if (!nametagged.get() && entity.hasCustomName()) return false;
        if (!PlayerUtils.canSeeEntity(entity) && NFPlayerUtils.distanceFromEye(entity) > wallsRange.get()) return false;
        ...
        switch (delayMode.get()) {
            ...
            case Custom -> {
                if (hitDelayTimer > 0) {
                    ...
                    hitDelayTimer = (int) (hitDelay.get() / NFServerUtils.getTPSMatch(TPSSync.get()));
                    return true;
                }
            }

            case Fixed -> {
                if (fixedHitTimer.passedMillis((long) (fixedDelay() / NFServerUtils.getTPSMatch(TPSSync.get())))) return true;
            }
        }
    ...
    private boolean itemInHand() {
        return switch (weapon.get()) {
            ...
            case Pick -> mc.player.getMainHandStack().getItem() instanceof PickaxeItem;
