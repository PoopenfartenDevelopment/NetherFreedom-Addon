...
import netherfreedom.utils.NFWorldUtils;
...
public class Strafe extends Module {
    ...
    public Strafe() {
        super(NetherFreedom.MAIN, "strafe", "Increase speed and control.");
    }
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (NFEntityUtils.isWebbed(mc.player) && webbedPause.get() == WebbedPause.Always) return;
        if (NFEntityUtils.isWebbed(mc.player) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;

        if (mc.player.isOnGround()) {
           timerClass.setOverride(PlayerUtils.isMoving() ? (groundTimer.get() * NFServerUtils.getTPSMatch(TPSSync.get())) : Timer.OFF);
        } else
           timerClass.setOverride(PlayerUtils.isMoving() ? (airTimer.get() * NFServerUtils.getTPSMatch(TPSSync.get())) : Timer.OFF);
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (NFWorldUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always) return;
        if (NFWorldUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
