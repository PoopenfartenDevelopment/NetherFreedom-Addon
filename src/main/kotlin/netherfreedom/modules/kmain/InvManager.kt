...
class InvManager:MeteorModule(NetherFreedom.MAIN, "InvManager", "Automatically organize your inventory") {
    ...
    private var slot0 by slotsGroup.add(SValue("Slot0",
                                               ...
                                               "",
                                               { false },
                                               { cur -> itemIds[0] = Identifier(cur).also { println("<InvManager> Loaded $it into slot0") } }))
    private var slot1 by slotsGroup.add(SValue("Slot1",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[1] = Identifier(cur).also { println("<InvManager> Loaded $it into slot1") } }))
    private var slot2 by slotsGroup.add(SValue("Slot2",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[2] = Identifier(cur).also { println("<InvManager> Loaded $it into slot2") } }))
    private var slot3 by slotsGroup.add(SValue("Slot3",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[3] = Identifier(cur).also { println("<InvManager> Loaded $it into slot3") } }))
    private var slot4 by slotsGroup.add(SValue("Slot4",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[4] = Identifier(cur).also { println("<InvManager> Loaded $it into slot4") } }))
    private var slot5 by slotsGroup.add(SValue("Slot5",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[5] = Identifier(cur).also { println("<InvManager> Loaded $it into slot5") } }))
    private var slot6 by slotsGroup.add(SValue("Slot6",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[6] = Identifier(cur).also { println("<InvManager> Loaded $it into slot6") } }))
    private var slot7 by slotsGroup.add(SValue("Slot7",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[7] = Identifier(cur).also { println("<InvManager> Loaded $it into slot7") } }))
    private var slot8 by slotsGroup.add(SValue("Slot8",
                                               "",
                                               "",
                                               { false },
                                               { cur -> itemIds[8] = Identifier(cur).also { println("<InvManager> Loaded $it into slot8") } }))

    private var delay by mainGroup.add(IValue("Delay", 1, "Delay between moving items in ticks.", 1..35, 1))
    ...
    override fun onActivate() {
        super.onActivate()
        println("<InvManager> ${itemIds.contentToString()}")
    }
    ...
    private fun save() {
        ...
        mc.inGameHud.chatHud.addMessage(Text.of("<${Formatting.LIGHT_PURPLE}InvManager${Formatting.RESET}> Saved inventory -> ${
